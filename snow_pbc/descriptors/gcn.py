from snow_pbc.lodispp.utils_pbc import pair_list, coordination_number
import numpy as np
...
def gcn_calculator(atoms : Atoms, cut_off, dbulk : list[float], strained: bool = False):
    """
    """
    neigh_list, coord_numbers = coordination_number(atoms, cut_off )
    n_atoms = len(coord_numbers)
    gcn = np.zeros(n_atoms)
    for i, atom_neighbors in enumerate(neigh_list):
        if strained:
            ...
            gcn[i]=((sgcn-self_sgcn)/gcn_max)
        else:
            gcn[i] = sum(coord_numbers[neigh] for neigh in atom_neighbors)# - coord_numbers[i]
    return gcn
